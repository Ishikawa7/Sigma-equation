...
def sigma_equation_c_code_2(cecita, complessita, disistima_espressa, disistima_ricevuta, rifiuto_effettivo):
    tensione0 = cecita + complessita
    diff = complessita - cecita
    diff1 = cecita - complessita
    coef = 1
    if complessita > cecita:
        coef = 1 - (1 + (1 - (diff ** 2))/diff ** 2) ** -1
    if complessita < cecita:
        coef = 1 - (1 + (1 - (diff1 ** 4))/(diff ** 2) ** 2) ** -1
    tensione0 = tensione0 * coef

    diff = disistima_espressa - cecita
    diff1 = cecita - disistima_espressa
    coef = 1
    if disistima_espressa > cecita:
        coef = 1 - (1 + (1 - (diff ** 2))/sqrt(diff ** 2)) ** -1
    if disistima_espressa < cecita:
        coef = 1 - (1 + (1 - (diff1 ** 4))/sqrt(diff ** 4)) ** -1
